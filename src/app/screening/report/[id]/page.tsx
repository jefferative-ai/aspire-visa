    ...
    <div className="min-h-dvh bg-[var(--bg)] flex flex-col">
      ...
      <header className="flex items-center justify-between px-6 md:px-12 py-5 border-b border-[var(--border)]">
        ...
        <a
          ...
          className="inline-flex items-center gap-2 h-11 px-4 bg-[var(--accent)] text-[var(--accent-fg)] font-semibold text-sm rounded-lg hover:bg-[var(--accent-hover)] transition-colors"
        >
      ...
      <main className="flex-1 max-w-2xl mx-auto w-full px-6 py-12 flex flex-col gap-10">
        ...
        <section className="flex flex-col gap-4">
          ...
          <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-lg bg-[var(--surface)] border border-[var(--border)]">
            <span className="text-xs text-[var(--text-3)]">Confidence</span>
            <span className="text-xs font-semibold text-[var(--text-1)]">{report.eligibility.confidence_level}</span>
          </div>
        </section>

        {/* Hard blocks */}
        {report.eligibility.hard_blocks.length > 0 && (
          <section className="bg-danger/8 border border-danger/25 rounded-2xl p-6 flex flex-col gap-4">
            ...
            <p className="text-xs text-[var(--danger)] opacity-70">
            ...
            <ul className="flex flex-col gap-3">
              {report.eligibility.hard_blocks.map((block, i) => (
                <li
                  key={i}
                  className="flex items-start gap-3 text-sm text-[var(--danger)]"
                >
                  <span className="mt-0.5 w-4 h-4 flex-shrink-0 flex items-center justify-center rounded-full bg-danger/15 text-[var(--danger)] text-xs font-bold">
                    !
                  </span>
                  {block}
                </li>
        ...
        {report.eligibility.risk_flags.length > 0 && (
          <section className="bg-warning/8 border border-warning/25 rounded-2xl p-6 flex flex-col gap-4">
            <div className="flex items-center gap-2.5">
            ...
            <p className="text-xs text-[var(--warning)] opacity-70">
            ...
            <ul className="flex flex-col gap-3">
              {report.eligibility.risk_flags.map((flag, i) => (
                <li
                  key={i}
                  className="flex items-start gap-3 text-sm text-[var(--warning)]"
                >
                  <span className="mt-0.5 w-4 h-4 flex-shrink-0 flex items-center justify-center rounded-full bg-warning/15 text-[var(--warning)] text-xs font-bold">
                    ▲
                  </span>
                  {flag}
                </li>
        ...
        {/* Action buttons — download first */}
        <div className="flex flex-wrap gap-3">
          <a
            href={`/api/report/${id}/pdf`}
            className="inline-flex items-center gap-2 h-11 px-6 bg-[var(--accent)] text-[var(--accent-fg)] font-semibold text-sm rounded-xl hover:bg-[var(--accent-hover)] transition-colors"
          >
        ...
        {showConsultation && (
          <section className="bg-[var(--text-1)] text-[var(--accent-fg)] rounded-2xl p-8 flex flex-col gap-4">
            ...
            <div className="flex items-center gap-4 flex-wrap">
              <a
                href={`mailto:hello@aspirevisapro.com?subject=${encodeURIComponent("Consultation Request — Aspire Visa Pro")}&body=${encodeURIComponent(`Hi,\n\nI would like to book a 1-hour consultation.\n\nReport ID: ${id}\nStatus: ${report.status}\n\nPlease send me available times.\n\nThank you.`)}`}
                className="inline-flex items-center justify-center h-12 px-8 bg-[var(--accent-fg)] text-[var(--accent)] font-bold rounded-xl hover:bg-[var(--surface-hover)] transition-colors text-sm"
              >
                Book Consultation — $70
              </a>
              <span className="text-xs opacity-50">Reply within 24 hours</span>
            </div>
          </section>
        )}
